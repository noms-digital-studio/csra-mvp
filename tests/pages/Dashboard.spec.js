...
describe('<Dashboard />', () => {
    context('Default Dashboard', () => {
        ...
        it('calls actions when component mounts', () => {
            ...
            const wrapper = mount(<Dashboard getViperScores={getViperScores} getOffenderNomisProfiles={getOffenderNomisProfiles} />);
    ...
    context('Connected Dashboard', () => {
        ...
        it('responds to profile selection', () => {
            ...
            expect(store.dispatch.calledWithMatch({ type: 'SELECT_OFFENDER', payload: profiles[0] })).to.be.true;
        });

        it('calls actions when component mounts', () => {
            expect(store.dispatch.calledWithMatch({ type: 'GET_VIPER_SCORES', payload: viperScores })).to.be.true;
            
            expect(store.dispatch.calledWithMatch({ type: 'GET_OFFENDER_NOMIS_PROFILES', payload: offenderProfiles.output })).to.be.true;
        });
    });
});